...
begin
  ...
  try
    // Campos não sensíveis: leitura direta
  ...
  // Campos sensíveis: leitura criptografada
...
begin
  ...
  Ini := TIniFile.Create(ConfigFile);
  try
    // Campos não sensíveis: gravação direta
  ...
  // Campos sensíveis: gravação criptografada (XOR + hex)
